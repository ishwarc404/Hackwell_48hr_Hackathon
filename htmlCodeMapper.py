def init():
    return "<html><script>;var batchIds = [];function batchUpdateValueType(t){batchIds.push(t);};function batchUpdateValueTypeBackend(valueType){var a=prompt('Please enter the value:'),n={path:batchIds,value:a, valueType: valueType};var e=new XMLHttpRequest();e.open('POST','http://23471c5816f6.ngrok.io/batchUpdate',true);e.setRequestHeader('Content-type','application/json');e.send(JSON.stringify(n))};function editInstruction(t){var a=prompt('Please enter the new task details:'),n={path:t,value:a};var e=new XMLHttpRequest();e.open('POST','http://23471c5816f6.ngrok.io/editInstruction',true);e.setRequestHeader('Content-type','application/json');e.send(JSON.stringify(n))};function refresh(){var e=new XMLHttpRequest();e.open('GET','http://23471c5816f6.ngrok.io/refreshPage',true);e.send();setTimeout(function(){document.location.reload();},3000)};function addValueUnit(t){var a=prompt('Please enter a new value which you want to include:'),n={path:t,value:a};var e=new XMLHttpRequest();e.open('POST','http://23471c5816f6.ngrok.io/addDataUnit',true);e.setRequestHeader('Content-type','application/json');e.send(JSON.stringify(n))};function deleteValueUnit(t){var a=prompt('Please enter the value which you want to remove:'),n={path:t,value:a};var e=new XMLHttpRequest();e.open('POST','http://23471c5816f6.ngrok.io/deleteDataUnit',true);e.setRequestHeader('Content-type','application/json');e.send(JSON.stringify(n))};function deleteValueType(t){var a=prompt('Please enter the type which you want to remove:'),n={path:t,value:a};var e=new XMLHttpRequest();e.open('POST','http://23471c5816f6.ngrok.io/deleteDataType',true);e.setRequestHeader('Content-type','application/json');e.send(JSON.stringify(n))};function addValueType(t){var a=prompt('Please enter the value type which you want to add:'),n={path:t,value:a};var e=new XMLHttpRequest();e.open('POST','http://23471c5816f6.ngrok.io/addDataType',true);e.setRequestHeader('Content-type','application/json');e.send(JSON.stringify(n))};</script><link rel='stylesheet' href='https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css'>"
...
def createInput(submodule, path):
    ...
    if(submodule["Value Required"]):
        ...
        value_reference_path+="Value Unit" #adding the index to the reference path
        field += "<button id='{}' style='margin-left:10px' class='btn btn-primary' onclick=addValueUnit(this.id)>Add Unit</button>".format(value_reference_path)
        field += "<button id='{}' style='margin-left:10px' class='btn btn-primary' onclick=addValueType(this.id)>Add Value</button>".format(value_reference_path)
